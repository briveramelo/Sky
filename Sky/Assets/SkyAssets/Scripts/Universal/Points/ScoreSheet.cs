...
public class ScoreSheet : Singleton<ScoreSheet>, ITallyable, IResetable, IReportable, IStreakable {
	...
	void IStreakable.ReportHit(int newHitWeaponNumber){
        int weaponNumberDif= newHitWeaponNumber-_lastHitWeaponNumber;
	...
	void IResetable.ResetWaveCounters(){
        ...
        for (int i=0; i<_scoreCounters.Count; i++){
    ...
    void IReportable.ReportScores() {
        if (WaveManager.CurrentWave == WaveName.Endless) {
            ...
            EndlessScore myEndlessScore = new EndlessScore(_scoreCounters[ScoreType.Total].GetCount(BirdType.All, false), duration);
            FindObjectOfType<SaveLoadData>().PromptSave(myEndlessScore);
        }
        else {
            StoryScore myStoryScore = new StoryScore(_scoreCounters[ScoreType.Total].GetCount(BirdType.All,false), WaveManager.CurrentWave);
